    ...
    <nav className={`fixed w-full z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          >
            <h1 className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 dark:from-blue-400 dark:to-purple-400 bg-clip-text text-transparent group-hover:scale-105 transition-transform">
              Portfolio
            </h1>
          </div>
            ...
            <div className="ml-10 flex items-baseline space-x-4">
              {navItems.map((item) => (
                <button
                  ...
                  className={`relative px-3 py-2 rounded-md text-sm font-medium capitalize transition-colors ${
                    activeSection === item
        ...
        <div className="px-2 pt-2 pb-3 space-y-1">
          {navItems.map((item) => (
            <button
              key={item}
              onClick={() => scrollToSection(item)}
              className={`w-full text-left px-3 py-2 rounded-md text-base font-medium capitalize transition-colors ${
                activeSection === item
